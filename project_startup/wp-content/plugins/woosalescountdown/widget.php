						...
						if ( trim( $product->product_type ) != 'variable' ) {
							...
							<?php } ?>
								...
								if ( $time_from && $time_end ) {
									...
									jQuery(function () {
										jQuery('.widget_myCounter_" . $product->id . "').mbComingsoon({ expiryDate: new Date(" . date( "Y", $time ) . ", " . ( date( "m", $time ) - 1 ) . ", " . date( "d", $time ) . "), speed:500, gmt:" . date( "Z" ) . $date_text . $localization . " });
									});
									</script>";
								}
