...
class WSCD {
					...
					if ( $current_time < $time_end ) {
						if ( $time_end && $time_from ) {
							...
							switch ( $ob_time_show ) {
								...
								<span>' . $sale . ' ' . __( 'sold', 'woosalescountdown' ) . '</span> </div>
                                ';
									}
									break;
								default:
									...
									if ( $check_sale ) {
										$shortcode .= '<div class="ob_discount"><div class="ob_sale" style="width:' . $per_sale . '%"></div></div>
								<span>' . $sale . ' ' . __( 'sold', 'woosalescountdown' ) . '</span>';
	...
	function single_product( $post_ID ) {
		...
		if ( trim( $_product->product_type ) != 'variable' ) {
			...
			switch ( $ob_time_show ) {
				...
				case 2:
					if ( $check_sale ) {
						$show = "<div class=\"ob_warpper ob_product_detal\">" . $message . "  <span>" . $sale . " " . __( 'sold', 'woosalescountdown' ) . "</span> <div class=\"ob_discount\"><div class=\"ob_sale\" style=\"width:" . $per_sale . "%\"></div></div></div>";
					} else {
						$show = '';
					}
				...
				default:
					...
					if ( $check_sale ) {
						$show .= "<div class=\"ob_discount\"><div class=\"ob_sale\" style=\"width:" . $per_sale . "%\"></div></div><span>" . $sale . " " . __( 'sold', 'woosalescountdown' ) . "</span> ";
					}
			...
			if ( count( array_filter( $_product_variables ) ) > 0 ) {
				...
				foreach ( $_product_variables as $_product_variable ) {
					...
					switch ( $ob_time_show ) {
						...
						case 2:
							if ( $check_sale ) {
								$show = "<div " . $style . " class=\"ob_warpper ob_product_avariable_detail ob_product_detail_" . $_product_variable['variation_id'] . "\">" . $message . " <span>" . $sale . " " . __( 'sold', 'woosalescountdown' ) . "</span> <div class=\"ob_discount\"><div class=\"ob_sale\" style=\"width:" . $per_sale . "%\"></div></div></div>";
							} else {
								$show = '';
							}
	...
	function categories_product() {
		...
		switch ( $ob_time_show ) {
			...
			case 2:
				if ( $check_sale ) {
					$show = "<div class=\"ob_warpper ob_categories\">" . $message . " <div class=\"ob_discount\"><div class=\"ob_sale\" style=\"width:" . $per_sale . "%\"></div></div><span>" . $sale . " " . __( 'sold', 'woosalescountdown' ) . "</span></div>";
				}
				break;
			default:
